...
```converter.py <--a_to_m> <init.in> <react.r> <--output> <MARlea_crn.csv> [waste=W] [aether=S.1,S.2,S.3]```

```converter.py <--m_to_a> <MARlea_crn.csv> <--output> <init.in> <react.r> [waste=garbo] [aether=S.1]```